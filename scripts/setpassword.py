...
class LongPasswordRule():
    ...
    def __init__(self, numChars):
        ...
        self.validationErrMsg = "{} rule failed: Password must be at least {} chars long".format(self.name, self.numChars)
...
class UppercaseCharPasswordRule():
    ...
    def __init__(self, numCharsMustSatisfyRule):
        ...
        self.validationErrMsg = "{} rule failed: Password should have at least {} chars that are uppercase".format(self.name,
...
class LowercaseCharPasswordRule():
    ...
    def __init__(self, numCharsMustSatisfyRule):
        ...
        self.validationErrMsg = "{} rule failed: Password should have at least {} chars that are lowercase".format(self.name,
                                                                                                 self.numCharsMustSatisfyRule)
...
class SpecialCharPasswordRule():
    ...
    def __init__(self, numCharsMustSatisfyRule):
        ...
        self.desc = "Password should have at least {} special characters ({})".format(self.numCharsMustSatisfyRule,
                                                                                      self.specialChars)
        self.validationErrMsg = "{} rule failed: Password should have at least {} special characters ({})".format(self.name,
                                                                                                 self.numCharsMustSatisfyRule,
                                                                                                self.specialChars)
...
class AlphaNumericCharPasswordRule():
    ...
    def __init__(self, numCharsMustSatisfyRule):
        ...
        self.validationErrMsg = "{} rule failed: Password should have at least {} alphanumeric characters".format(self.name,
                                                                                                 self.numCharsMustSatisfyRule)
    ...
    def validate(self, password):
        ...
        resp = {
            "passed" : (len(alphabeticalChars) >= self.numCharsMustSatisfyRule and len(numericalChars) >= self.numCharsMustSatisfyRule)
        }
        return resp
...
class CharacterCategoriesPasswordRule():
    ...
    def __init__(self, numSubRulesMet):
        ...
        self.desc = "Password must meet at least {}  of the following subrules: {} ".format(self.numSubRulesMet,
                                                                                            [ r.name for r in self.subRules])
        self.validationErrMsg = "{} rule failed: Password must meet at least {}  of the following subrules: {}".format(self.name,
...
def setPassword(projectId, userName, dbName, newPass):
    """
    ...
    :return:
    """
    logging.info("Setting password for user {} on db {}".format(userName, dbName))
...
def setupArgs():
    ...
    parser.add_argument('--dryRun',                   required=False, action="store_true", dest='dryRun',          default=False,                 help='Include this flag to test out the password without actually creating it.')
    parser.add_argument('--userName',                 required=False, action="store", dest='userName',             default=None,                 help='The user whose password to reset/set')
    parser.add_argument('--dbName',                 required=False, action="store", dest='dbName',             default=None,                    help='Name of the db to which the user belongs')
...
def main():
    ...
    passwordRules = [
        LongPasswordRule(16),
        CharacterCategoriesPasswordRule(3)
    ]

    validatedPass = captureAndValidatePassword(passwordRules, args.userName)
    dryRun = False if args.dryRun is None else args.dryRun
    if not dryRun:
        logging.info("Setting password")
        setPassword(args.projectId, args.userName, args.dbName, validatedPass)
    else:
